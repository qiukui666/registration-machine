#!/usr/bin/env python3
"""
GPT Register - GPT/OpenAI 账号注册模块
使用 Selenium 自动化注册流程
集成人类鼠标轨迹模拟和反检测功能
"""
...
class GPTRegister:
    ...
    # 可用于验证邮件的关键字
    VERIFY_KEYWORDS = ["openai", "激活", "验证"]
    # 必须包含的域名关键字（避免匹配 Cloudflare 等无关邮件）
    VERIFY_DOMAINS = ["openai.com", "ai.com", "chatgpt.com"]
    ...
    def register(self, email_info: Dict) -> Optional[Dict]:
        ...
        for attempt in range(1, self.max_retries + 1):
            self.logger.info(f"开始注册 GPT 账号: {email} (尝试 {attempt}/{self.max_retries})")
            ...
            try:
                ...
                if account_info:
                    ...
                    self.logger.info(f"✅ GPT 账号注册成功: {email}")
                    return account_info
    ...
    def _fill_registration_form(self, email: str) -> Optional[Dict]:
        ...
        try:
            ...
            if password_input:
                self._human_type_with_move(password_input, password)
    ...
    def _check_registration_success(self, email: str, password: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 成功标志：跳转到 dashboard/auth/chat 等页面
            success_indicators = ["dashboard", "auth", "chat", "playground"]
            failure_indicators = ["login", "signup"]
    ...
    def _log_page_errors(self) -> None:
        ...
        error_selectors = [
            ...
            "[data-error]", "[class*='error']", ".invalid-feedback",
        ]
...
def register_gpt_account(
    ...
    if getattr(args, "verify", False):
        ...
        if not register.verify_email(email_info):
            config.logger.warning("邮箱验证失败，不保存账号")
            register.teardown_driver()
            return None
