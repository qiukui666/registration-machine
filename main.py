...
def _register_single(
    ...
    Returns:
        (success, fail, account_info)
    """
    config.logger.info("")
    config.logger.info(f"[{platform.upper()}] [{index}/{total}] 开始注册...")

    # 每次注册前切换代理节点
    import subprocess
    try:
        result = subprocess.run(
            ["python", "clash_switch.py"],
            capture_output=True,
            text=True,
            timeout=10,
        ...
        if result.returncode == 0:
            config.logger.info(f"  节点轮换: {result.stdout.strip()}")
        else:
            config.logger.warning(f"  节点轮换失败: {result.stderr.strip()}")
    except Exception as e:
        config.logger.warning(f"  节点轮换异常: {e}")
    ...
    account = register_fn(args, config, email_gen)
...
def run_platform(
    ...
    if concurrency > 1:
        # 并发模式：使用 ThreadPoolExecutor
        config.logger.info(f"[{platform.upper()}] 启动并发模式，并发数: {concurrency}")

        # 每个线程需要独立的 Config 副本（避免 logger 冲突）
        def make_config():
            c = Config(args.config)
            c.config_data = config.config_data.copy()
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            ...
            for future in as_completed(futures):
                try:
                    s, f, account = future.result()
                    success += s
                    fail += f
                    if account:
                        _save_accounts(account, args, platform)
                except Exception as e:
                    config.logger.error(f"任务执行失败: {e}")
                    fail += 1
        ...
        success, fail = 0, 0
        for i in range(1, args.max_accounts + 1):
            s, f, account = _register_single(
                platform, args, config, email_gen, i, args.max_accounts
            )
            success += s
            fail += f
            if account:
...
def main():
    ...
    config.logger.info(f" 并发数: {args.concurrency}")
    config.logger.info("=" * 60)
